...
function useDrivePicker() {
    ...
    var _c = react_1.useState(false), openAfterAuth = _c[0], setOpenAfterAuth = _c[1];
    var _d = react_1.useState(false), authWindowVisible = _d[0], setAuthWindowVisible = _d[1];
    var _e = react_1.useState(typeDefs_1.defaultConfiguration), config = _e[0], setConfig = _e[1];
    var _f = react_1.useState(), authRes = _f[0], setAuthRes = _f[1];
        ...
        if (authWindowVisible) {
            window.gapi.auth.authorize({
                ...
                scope: config.customScopes
                    ? __spreadArray(__spreadArray([], defaultScopes), config.customScopes) : defaultScopes,
                immediate: false,
            }, handleAuthResult);
        }
    }, [authWindowVisible]);
